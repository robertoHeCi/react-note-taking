...
describe('apiService', () => {
  ...
  describe('getNoteById', () => {
    ...
    it('should throw error when note is not found', async () => {
      await expect(apiService.getNoteById('999')).rejects.toThrow('Failed to fetch note with id: 999');
    });
  });

  describe('createNote', () => {
    const newNote = { id: '1', body: 'New note' };
    ...
    it('should create a note successfully', async () => {
      ...
      expect(createdNote.body).toBe(newNote.body);
    });
    ...
    it('should throw error when creation fails', async () => {
      ...
      await expect(apiService.createNote(newNote)).rejects.toThrow('Failed to create note');
    });
  });
}); 